/*
 * AVP_menu - Allow using props without using sitting scripts.
 *
...
default
{
    ...
    listen(integer listen_channel, string name, key id, string msg)
    {
        if (choice != "")
        {
            ...
            else
            {
                ...
                {
                    llMessageLinked(LINK_THIS, 90173, msg, choice); // add PROP line to AVP_prop
    ...
    link_message(integer sender, integer num, string msg, key id)
    {
        if (sender == llGetLinkNumber() || LMSOURCE == 1)
        {
            ...
            else if (num == 90022) // send dump to AVP_adjuster
            {
                Readout_Say(msg);
            }
