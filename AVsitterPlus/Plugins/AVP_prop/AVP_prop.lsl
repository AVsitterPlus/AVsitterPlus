/*
 * AVP_prop - Rez props when playing poses
 *
...
remove_props_by_group(integer gp)
{
    ...
    if (text != "")
    {
        if (llGetInventoryType(main_script) == INVENTORY_SCRIPT)
        {
            // This is AVP_sitA/B, send the command to all sitters
            send_command("REM_INDEX" + text);
        }
        else
        {
            // Presumed to be launched from AVP_menu; avoid removing attachments from others
...
{
    llSleep(0.2);
    llMessageLinked(LINK_THIS, 90022, say, ""); // dump to AVP_adjuster
}
...
default
{
    ...
    link_message(integer sender, integer num, string msg, key id)
    {
        if (sender == llGetLinkNumber())
        {
            ...
            if (num == 90171 || num == 90173) // AVP_adjuster/AVP_menu add PROP line
            {
                integer sitter;
                if (num == 90171) // AVP_adjuster?
                {
                    sitter = (integer)msg;
